...
namespace PointCloudExporter
{
	public class PointCloudGenerator : MonoBehaviour
	{
		...
		public int maximumVertices = 6000000;
		...
		private const int verticesMax = 128*128;
		...
		public MeshInfos LoadPointCloud (string pointFilePath)
		{
			//string filePath = System.IO.Path.Combine(Application.streamingAssetsPath, fileName) + ".ply";
			return SimpleImporter.Instance.Load(pointFilePath, maximumVertices);
		}

		public void Generate (string pointFilePath,  Material materialToApply, GameObject parentObj, out MeshInfos out_points)
		{
            //Debug.Log(pointFilePath);
			points = LoadPointCloud(pointFilePath);
			out_points = points;

			materialToApply.SetFloat("_Size", size);
			//material = new Material(shader);
			//material.SetTexture("_MainTex", _sprite);
			Generate(points, materialToApply, MeshTopology.Points, parentObj);
		}
		...
		public void Generate (MeshInfos meshInfos, Material materialToApply, MeshTopology topology, GameObject parentObj)
		{

			for (int c = parentObj.transform.childCount - 1; c >= 0; --c) {
				Transform child = parentObj.transform.GetChild(c);
